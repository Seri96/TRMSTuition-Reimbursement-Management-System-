...
class FormDAOImpl(FormDAO):
    ...
    def create_form(self, form):
        sql = "INSERT INTO form VALUES(default, %s, %s, %s, %s, %s, %s, %s, %s, %s, 1, %s) RETURNING *"
        cursor = connection.cursor()
        cursor.execute(sql, (form.employee_id, form.date, form.time, form.location, form.description, float(
            form.cost), form.grading_format, form.type_event, form.justification, form.denied_reason))
        connection.commit()
        record = cursor.fetchone()
        return Form(record[0], record[1], record[2], record[3], record[4], record[5], record[6], record[7], record[8], record[9], record[10], record[11])
    ...
    def get_nonapproved_forms(self):
